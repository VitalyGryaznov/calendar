...
const getDaysFromTheCurrentMonth = (year: number, month: number): Array<string> =>
    Array(getNumberOfDaysInMoth(year, month)).fill(0).map((_, i) => dayjs(new Date(year, month, i + 1)).format(calendarDateFormat));

const getDaysFromThePreviousMonthAtTheFirstWeekOfTheCurrentMonth = (year: number, month: number): Array<string> => {
    ...
    reverse().
    map((day) => dayjs(new Date(year, month - 1, day)).format(calendarDateFormat));
}
    
const getDaysFromTheNextMonthAtTheLastWeekOfTheCurrentMonth = (year: number, month: number): Array<string> => {
    ...
    fill(0).
    map((_, i) => dayjs(new Date(year, month + 1, i + 1)).format(calendarDateFormat));
}
